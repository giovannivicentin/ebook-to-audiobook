...
function resolveVoiceForUtterance(
  ...
  const byLang = pool.find(
    (x) => x.lang.toLowerCase().replace("_", "-") === wantLang.toLowerCase(),
  );
...
export function useVoices(mode: LangMode) {
  const filter = useCallback(
    (list: SpeechSynthesisVoice[]) =>
      sortVoicesNaturalFirst(filterVoicesByMode(list, mode)),
    [mode],
  );
  ...
  const [voices, setVoices] = useState<SpeechSynthesisVoice[]>(() =>
    typeof window === "undefined"
      ? []
      : filter(window.speechSynthesis.getVoices()),
  );
  ...
  useEffect(() => {
    ...
    synth.onvoiceschanged = sync as
      | ((this: SpeechSynthesis, ev: Event) => void)
      | null;
    return () => {
      if (synth.onvoiceschanged === sync) {
...
export function useSpeechQueue(
  ...
  const speak = useCallback((chunks: string[], startIndex = 0) => {
    ...
    const idx = Math.max(
      0,
      Math.min(startIndex, Math.max(0, chunks.length - 1)),
    );
  ...
  const speakFromFullText = useCallback(
  ...
  const seekToChunk = useCallback(
    (zeroBasedIndex: number, resumePlayback: boolean) => {
